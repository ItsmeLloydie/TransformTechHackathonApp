...
export default function ConcernDetailDrawer({ isOpen, onClose, concern, currentUser }: DrawerProps) {
  ...
  const suggestReply = async () => {
    ...
    const prompt = `You are an AI assistant helping staff from the ${concern.department} department at a university in the Philippines. Your task is to draft a response to a student concern.
...
STEP 1: EVALUATE MISMATCH
You must aggressively analyze if the 'Title', 'Category', and 'Student Description' logically match the primary responsibilities of the 'Department Handling' (${concern.department}).
Strict Rules:
- If the issue involves tuition, fees, payment, or money (e.g., "bayad tuition"), it MUST be handled by the Finance/Accounting department. If the department is anything else (like Student Affairs or Registrar), it is a MISMATCH.
- If the issue involves grades, subjects, or enrollment, it MUST be handled by the Registrar or Academic department. If sent to Finance, it is a MISMATCH.
If the 'Department Handling' does not perfectly align with the core subject of the concern, it is a MISMATCH.

IF THERE IS A MISMATCH:
You must output EXACTLY the word MISMATCH and absolutely nothing else. Stop generating immediately.

IF THERE IS NO MISMATCH:
Draft a meaningful, helpful, and empathetic official response to the student based on the following instructions:
...
REQUIREMENTS FOR STANDARD RESPONSE:
1. Opens by directly acknowledging the specific concern by title
2. Clearly states the current status of their concern
3. Gives specific practical advice relevant to their category
4. States a realistic next step or expected timeline
5. Ends with an encouraging closing
6. EXACTLY 4 to 5 sentences total
7. Professional but warm and human tone
8. Write in paragraph form, no bullet points
9. Do not make up specific names, amounts, or exact dates
10. Complete all sentences fully before stopping.`
    ...
    for (const modelId of candidateModels) {
      try {
        ...
        if (cleanSuggestion.startsWith('MISMATCH') || cleanSuggestion.includes('MISMATCH')) {
          setNoteText(`The details of this concern do not appear to align with the ${concern.department} department. This concern has been flagged and needs to be reviewed by the ${concern.department} admin for proper evaluation or re-routing.`);
        } else {
          setNoteText(cleanSuggestion);
        }
