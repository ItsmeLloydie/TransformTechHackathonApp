  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="bg-white border border-slate-200 rounded-2xl shadow-xl p-8">
        ...
        {showDraftBanner && (
          <div className="mb-6 p-4 bg-indigo-50 border border-indigo-100 rounded-2xl flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <button onClick={handleRestoreDraft} className="text-xs px-3 py-1.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl transition-colors shadow-sm">Restore</button>
              <button onClick={handleDiscardDraft} className="text-xs px-3 py-1.5 border border-indigo-200 text-indigo-600 hover:bg-indigo-100 rounded-xl transition-colors">Discard</button>
            </div>
          </div>
        )}

        {successId && (
          <div className="mb-8 p-4 bg-green-50 border border-green-100 rounded-2xl flex items-start space-x-3">
        ...
        {submitError && (
          <div className="mb-8 p-4 bg-red-50 border border-red-100 rounded-2xl flex items-start space-x-3">
        ...
        <form className="space-y-6" onSubmit={handleSubmit}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {!isAnonymous && (
              <div>
                ...
                <input 
                  ...
                  value={user.fullName}
                  readOnly
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-slate-900 opacity-60 cursor-not-allowed"
                />
              </div>
            )}
            ...
            <div className={`grid grid-cols-1 md:grid-cols-3 gap-4 p-5 border rounded-2xl mb-6 transition-all duration-300 ${isAnonymous ? 'bg-indigo-50 border-indigo-100 opacity-70' : 'bg-slate-50 border-slate-200'}`}>
              <div className="md:col-span-1">
                <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5">Full Name</label>
                <div className="text-slate-900 font-bold bg-white px-3 py-2 rounded-xl border border-slate-100 text-sm shadow-sm">
              ...
              <div className="md:col-span-1">
                ...
                <div className="text-slate-900 font-bold bg-white px-3 py-2 rounded-xl border border-slate-100 text-sm truncate shadow-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <input 
                ...
                className={`w-full bg-white border ${errors.email ? 'border-red-500' : 'border-slate-200'} rounded-xl px-4 py-2.5 text-slate-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-colors shadow-sm`}
              />
          ...
          <div className="space-y-6">
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div 
                  ...
                  className={`bg-white border rounded-2xl p-4 cursor-pointer transition-all shadow-sm ${category === 'Academic' ? 'border-indigo-600 bg-indigo-50 ring-1 ring-indigo-100' : 'border-slate-200 hover:border-indigo-300'}`}
                >
                ...
                <div 
                  ...
                  className={`bg-white border rounded-2xl p-4 cursor-pointer transition-all shadow-sm ${category === 'Financial' ? 'border-indigo-600 bg-indigo-50 ring-1 ring-indigo-100' : 'border-slate-200 hover:border-indigo-300'}`}
                >
                  <div className="flex items-center gap-3 mb-2">
                ...
                <div 
                  ...
                  className={`bg-white border rounded-2xl p-4 cursor-pointer transition-all shadow-sm ${category === 'Welfare' ? 'border-indigo-600 bg-indigo-50 ring-1 ring-indigo-100' : 'border-slate-200 hover:border-indigo-300'}`}
                >
                  <div className="flex items-center gap-3 mb-2">
            ...
            {category && (
              <div className="animate-in fade-in duration-200">
                ...
                <select 
                  ...
                  className="w-full bg-white border border-slate-200 rounded-xl px-4 py-2.5 text-slate-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-colors appearance-none shadow-sm"
                >
            ...
            <div>
              ...
              <input 
                ...
                className={`w-full bg-white border ${errors.title ? 'border-red-500' : 'border-slate-200'} rounded-xl px-4 py-2.5 text-slate-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-colors shadow-sm`}
              />
          ...
          <div>
            ...
            <textarea 
              ...
              className={`w-full bg-white border ${errors.description ? 'border-red-500' : 'border-slate-200'} rounded-xl px-4 py-3 text-slate-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-colors resize-y shadow-sm`}
            ></textarea>
                ...
                {fileUrl ? (
                  <>
                    ...
                    <button 
                      ...
                      className="mt-4 text-xs text-red-600 hover:text-red-700 font-black uppercase tracking-widest z-30 relative py-1 px-3 bg-red-50 rounded-xl border border-red-100 hover:bg-red-100 transition-all active:scale-95 shadow-sm"
                    >
                      Remove File
                    </button>
                  </>
          ...
          <div className="flex flex-col-reverse sm:flex-row justify-end gap-4 pt-4">
            <div className="flex items-center justify-between sm:justify-end w-full sm:w-auto">
              ...
              <button 
                ...
                className="px-6 py-3 border border-slate-200 text-slate-600 rounded-2xl hover:bg-slate-50 font-bold text-sm transition-all active:scale-95 w-full sm:w-auto shadow-sm"
                disabled={loading}
              >
                Save draft
              </button>
            ...
            <button 
              type="submit"
              disabled={loading}
              className="px-8 py-3 bg-indigo-600 disabled:opacity-50 disabled:cursor-not-allowed hover:bg-indigo-500 text-white rounded-2xl font-bold text-sm transition-all shadow-lg shadow-indigo-500/20 flex items-center justify-center active:scale-95 w-full sm:w-auto"
            >
              {loading ? (
